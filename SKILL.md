...
| # | 反模式 | 为什么不要做 | 正确做法 |
|---|--------|-------------|---------|
| 1 | **中途停下來問用戶**「要不要標點／要不要歸檔／這是什麼格式」 | 格式自動檢測、年號庫自我進化是工具設計目標；問了反而打斷全自動管線 | 直接跑；只有工具連起手格式都打不開才求助 |
| 2 | **改動標題行正文**（`### …` 年份標題內除標點外的任何字） | 標題是結構錨點，改動會破壞三錨點一致性與檢索 | 標題一個字都不許改，標點規則第 7 條明確禁止 |
| 3 | **漏保留出生句動詞**（「{名}生於/生于/生/誕」） | 首年正文是推定出生年的唯一證據，刪掉動詞後檢查報告無法核驗 | 標題只借日期顯示，正文出生句原樣保留（規則 6） |
| 4 | **在 SKILL.md 補充易變事實**（版本號、部數、案例清單、更新日誌） | SKILL.md 是入口不是全文，兩處不一致即污染 | 一律寫進 README（規則 1） |
| 5 | **讓 learnings 未過閘門就生效**（直接把自我進化條目當規則用） | pending 條目未經回歸驗證，套用可能污染後續處理 | 先 `--verify-learnings` 逐條臨時套用→回歸→PASS 才轉正（規則 5） |
| 6 | **改動腳本後跳過回歸測試** | 腳本改動可能讓歷史成績退化，無回歸即盲改 | 跑 `python tools/regression.py --run`，全部 PASS 才收工（規則 4） |
| 7 | **自動修正無參照的僵局**（無年齡、干支歧義、無 CBDB、共識不足） | 硬猜會產生錯誤年序，比留著可疑更糟 | 把該單項拿出來請用戶裁決，其餘可自動部分先做完（策略 5） |
| 8 | **把「供人工確認的清單」當作中斷信號**（缺年清單、verify 內容差異、OCR 候選） | 這些是結尾報告的內容，不是叫你中途停下 | 集中收進末尾一次性總結報告（策略 4） |
| 9 | **用 `git reset --hard` 回滾** | 丟工作樹未提交改動、破壞歷史 | 用 `nianpu_processor.py --revert`（.bak 單槽備份）或 `git revert` |
